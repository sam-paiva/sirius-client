...
const JobCard: React.FC<Props> = ({ job }) => {
  ...
  return (
    <Card className="mb-2 h-[150px] rounded-xl shadow-sm">
      <CardHeader className={`flex gap-3 ${job.userBundle.sponsored && 'bg-orange-200'} bg-cyan-900 rounded-xl`}>
          ...
          <div className="flex justify-between w-full rounded-xl">
            <div className="flex justify-between w-full rounded-xl">
              <div className="flex justify-start items-center gap-4 rounded-xl">
                ...
                <h1 onClick={() => navigate(`/position-details/${job.id}`)} className="text-2xl text-default font-bold cursor-pointer">
                  {job.title}
                </h1>
              </div>
              <div className="flex gap-1 rounded-xl">
                <Badge fontColor="text-default-500" color="bg-white" content={moment(job.createdDate).fromNow()} />
      ...
      <Divider />
      <CardBody className="h-48 rounded-xl">
        ...
        <div className="flex items-end justify-start h-full">
          ...
          <div className="bg-[#6787AD] max-w-[100px text-tiny text-default rounded-3xl px-2 py-1 m-1">{job.budget}</div>
        </div>
      </CardBody>
      <Divider />
    </Card>
